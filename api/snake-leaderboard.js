...
const KEY = 'dogged:snake-leaderboard';
...
function verifyPayload(data, signature) {
  ...
  const str = `SNAKE|${data.name}|${data.score}|${data.ts}|${data.pid}|${CLIENT_SECRET}`;
...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    if (req.method === 'POST') {
      ...
      // Rate limit
      if (byPid && (Date.now() - byPid.updatedAt) < 15000) {
      ...
      // Sort by score descending (highest first), keep top N
